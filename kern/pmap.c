...
page_init(void)
{
	...
	// free pages!
	size_t i, mp = MPENTRY_PADDR/PGSIZE;
	pages[0].pp_ref = 1;
	...
	uint32_t end = PADDR(boot_alloc(0));
	for (i = IOPHYSMEM/PGSIZE; i < end/PGSIZE; i++) {
		pages[i].pp_ref = 1;
	}
	for (i = end/PGSIZE; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	int i;
	int e = ROUNDUP(size, PGSIZE) / PGSIZE;
	for (i = 0; i < e; i++) {
		pte_t *ptep = pgdir_walk(pgdir, (void *)va, 1);
		if (!ptep)
			panic("no enough memery"); 
		*ptep = pa | perm | PTE_P;
		va += PGSIZE;
		pa += PGSIZE;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	if (start >= ULIM || end >= ULIM || end < start) {
		user_mem_check_addr = start > ULIM? start: ULIM;
		return -E_FAULT;
	}
	while(start < end) {
		pte_t *ptep = pgdir_walk(env->env_pgdir, (void *)start, 0);
		if (!ptep || !(*ptep & perm)) {
			user_mem_check_addr = start == (uint32_t)va ? start: ROUNDDOWN(start, PGSIZE);
			return -E_FAULT;
		}
		start += PGSIZE;
	}
